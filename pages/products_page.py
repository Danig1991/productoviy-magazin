...
class ProductsPage(ActionWithElement):
    ...
    def get_title_products(self):
        return self.visibility_of_element_located(self.TITLE_PRODUCTS_LOCATOR, "Продукты").text

    @allure.step("Получить имя тестового продукта")
    def get_name_test_product(self):
        return self.visibility_of_element_located(self.TEST_PRODUCT_LOCATOR, "Тестовый").text
    ...
    def get_price_test_product(self):
        text_price_test_product = self.visibility_of_element_located(
            self.PRICE_TEST_PRODUCT_LOCATOR,
            "Цена тестового продукта"
        ).text
        logging.info(f"Найдена цена тестового продукта - '{text_price_test_product}'.")
    ...
    def select_dropdown_option(self, value):
        dropdown = self.visibility_of_element_located(self.DROPDOWN_LOCATOR, "Выпадающий список")

        select = Select(dropdown)
        select.select_by_visible_text(value)
        logging.info(f"В выпадающем списке выбрано значение: '{value}'.")

    @allure.step("Получить список цен продуктов")
    def get_list_of_product_prices(self):
        price_elements = self.visibility_of_all_elements_located(
            self.PRICES_OF_PRODUCTS_LOCATOR,
            "Цены продуктов"
        )
        prices_of_products = []
    ...
    def click_filter_menu(self):
        filter_button = self.visibility_of_element_located(self.FILTER_BUTTON_LOCATOR, "Меню фильтров")
        self.click_button(filter_button)
        if "true" in filter_button.get_attribute("aria-expanded"):
            logging.info("Меню фильтров открыто.")
        else:
            logging.info("Меню фильтров закрыто.")
    ...
    def checkbox_activation(self, value, activate):
        ...
        if action_needed:
            self.click_button(checkbox_element)
        ...
        logging.info(f"Категория: '{value}\' - {action_text} {status}.")

    @allure.step("Получить количество карточек продукта")
    def get_product_card_count(self):
        return len(self.find_elements(self.PRODUCT_CARDS_LOCATOR[0], self.PRODUCT_NAMES_LOCATOR[1]))
    ...
    def set_price_filter_range(self, min_price, max_price):
        ...
        set_price(self.PRICE_FROM_LOCATOR, "Цена: от", min_price)
        set_price(self.PRICE_TO_LOCATOR, "Цена: до", max_price)
        logging.info(f"Установлен диапазон цены от {min_price} до {max_price}.")
    ...
    def set_product_quantity(self, product_name, target_quantity):
        ...
        if current_quantity == target_quantity:
            logging.info(f"Количество продукта '{product_name}' - {current_quantity} шт.")
            return
        ...
        while True:
            ...
            if new_quantity == target_quantity:
                logging.info(f"Количество '{product_name}' изменено: с {current_quantity} шт. на {new_quantity} шт.")
                break
    ...
    def decrease_product_quantity(self, product_name, decrease_by):
        ...
        new_quantity = self.get_product_counter_value(product_name)
        logging.info(f"Количество '{product_name}' уменьшено на {decrease_by}. Текущее: {new_quantity} шт.")
